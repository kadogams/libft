...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/03/06 11:56:38 by skadogam          #+#    #+#             */
/*   Updated: 2019/03/06 17:42:49 by skadogam         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
** similaire a ldi sans le % IDX_MOD
*/
...
int		op_lldi(t_vm *vm, t_pc *pc, int arg[3], char arg_type[3])
{
	...
	{
		if (vm->opt_v & 4)
			verbosity_4(*pc, g_op[14], pc->arg, pc->arg_type);
		if (vm->opt_v & 16)
		...
		pc->reg[arg[2]] = arg_read(vm->arena, pc->index + a + b, REG_SIZE);
	}
	pc->index += pc->arg_len + 1;
	return (OK);
}
